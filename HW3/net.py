...
def main():
  ...
  # Q8 Evaluate on Test
  ################################
  save_results(net, X_test, batch_size)


def save_results(model, X_val, batch_size):
  i=0
  lossFunc = CrossEntropySoftmax()
  with open(filename,'w', newline='') as out:
    csv_out = csv.writer(out)
    csv_out.writerow(['id','digit'])
    while i < len(X_val):
      X_batch = X_val[i:i + min(batch_size, len(X_val)-i)]
      k = i
      for result in np.argmax(model.forward(X_batch), axis=1):
            csv_out.writerow(str(k) + str(result))
            k += 1
      i+=batch_size
...
class LinearLayer:
  ...
  #for backward : the gradient is in dL/dZ
  # self.grad_weights = dL/dW as in Eq. 32
  # self.grad_bias    = dL/db as in Eq 27
